...
func main() {
	...
	trillianAddress := flag.String(
		"logaddress",
		"localhost:50054",
		"The trillian RPC logger address for PUF entry transparency (e.g. 10.168.1.101:50054)")
	treeID := flag.Int64(
		"treeid",
		0,
		"The merkle tree ID that stores the PUF entries (e.g. 10.168.1.101:50054)")
	flag.Parse()
	if *configFile == "" {
		flag.Usage()
		os.Exit(1)
	...
	chainLength := 2 // 4-level chain: Root CA + 2 * Intermediate CA + Leaf CA
	if val, err := strconv.ParseInt(os.Getenv("PEBBLE_CHAIN_LENGTH"), 10, 0); err == nil && val >= 0 {
		chainLength = int(val)
	}
	logger.Printf("Current Chain Length (default 2 here): %d", chainLength)
	logger.Printf("Listening on %s for trillian server.", *trillianAddress)

	//*treeID = 0
	trclient.InitTrillainClient(*trillianAddress, *treeID)
